...
const CombinedAnimatedFeatures = () => {
  ...
  const handleClick = (id: number) => {
    ...
    setTimeout(() => {
      ...
      }, 500); // delay before showing description
    }, 1000); // delay for name to settle
  };

  const handleReset = () => {
    // First hide description, then animate name back
    setShowDescription(false);
    setTimeout(() => {
      setIsExiting(true);
      setIsSettled(false);
    }, 1000); // Wait for description to slide out
  };
  ...
  return (
    ...
    >
      <div className="flex w-full  transition-all duration-500">
        ...
        {/* Right Side */}
        <div className={`${activeId ? "w-full" : "w-1/2"} relative`}>
          {/* Partner Title List */}
          {!activeId &&
            partners.map((partner) => (
              ...
              >
                <span className="text-[27px] text-[#101010] group-hover:text-orange-500 font-[Questrial,Arial,Verdana,Tahoma,sans-serif] font-normal transition-colors duration-200">
                  {partner.title}
                </span>
          ...
          {/* Floating Partner Title */}
          <AnimatePresence onExitComplete={handleExitComplete}>
            {activeId && activePartner && !isExiting && (
              <motion.div
                key={activeId}
                initial={{ x: 0, y: 0, opacity: 1 }}
                animate={
                  isSettled
                    ? { x: "-35vw", y: 750, opacity: 1 }
                    : { x: "-35vw", y: 0, opacity: 1 }
                }
                exit={{
                  x: ["-35vw", "-35vw", "50%"],
                  y: [750, 0, 0],
          ...
          <AnimatePresence>
            {showDescription && activePartner && !isExiting && (
              ...
              >
                <div className="flex w-3/4 flex-col md:flex-row gap-10 items-start justify-around">
                  {/* Left Side: Large Image */}
                  <div className="md:w-1/2 w-full flex justify-center">
                    <img
                      src={activePartner.image}
                      alt={activePartner.title}
                      className="w-full max-w-md h-auto  shadow-md object-cover"
                    />
                  </div>
